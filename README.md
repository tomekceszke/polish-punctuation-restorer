...
- The website runs the same model client-side: `src/utils/export_web.m` writes the weights as a flat
  float32 blob and `web/model.js` replays the `detect.m` pipeline in JavaScript — no server, no ML
  library. See [`notes/web-demo.md`](notes/web-demo.md).
...
│       ├── export_web.m     # Export weights as float32 binary + vocab.txt + meta.json for web/
...
│   └── web-demo.md               # Browser demo: weight export, JS port, parity with detect.m
...
├── web/                          # Landing page (GitHub Pages) — runs the model in the browser
│   ├── model.js                  # JS port of the inference path: loads the weights, runs the forward pass
│   └── model/                    # Generated by export_web.m: weights.bin, vocab.txt, meta.json
├── archive/
│   └── web-v1/                   # The retired first landing page, kept for reference (not deployed)
```
